...
void botplay(void) {
	int sthlh = -1, sthlh2 = -1, k = N, i, j;
	while (1) {
		//orizontios elegxos
		for (i = 0; i < M; i++) {
			counter = 0;
			for (j = 0; j < N; j++) counter = Q[i][j] == '+' ? counter + 1 : 0;
			if (counter == 3) {
				if (sthlh == 2) sthlh = 3;
				else if (sthlh == N - 3) sthlh = N - 4;
				else {
					sthlh = i + 1;
					sthlh2 = i - 3;
				}
			...
			for (i = 0; i < M; i++) {
				counter = 0;
				for (j = 0; j < N; j++) counter = Q[j][i] == '+' ? counter + 1 : 0;
				if (counter == 3) sthlh = i;
			}

			if (Q[0][sthlh] == paula) break;
		}
		while (--k >= 0) if (Q[k][sthlh] == paula) {
			Q[k][sthlh] = '+';
			break;
		}
		puts("Player: Bot");
